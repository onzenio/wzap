...
func New(cfg config.Config, log *slog.Logger, deps Deps) *http.Server {
	...
	api.Handle("POST /api/v1/instances/{id}/messages/text", Idempotency(deps.Idempotency, log, cfg.MaxMediaBytes)(handleSendText(deps.Messages)))
	api.Handle("POST /api/v1/instances/{id}/messages/location", Idempotency(deps.Idempotency, log, cfg.MaxMediaBytes)(handleSendLocation(deps.Messages)))
	api.Handle("POST /api/v1/instances/{id}/messages/contact", Idempotency(deps.Idempotency, log, cfg.MaxMediaBytes)(handleSendContact(deps.Messages)))
	api.Handle("POST /api/v1/instances/{id}/messages/media", Idempotency(deps.Idempotency, log, cfg.MaxMediaBytes)(handleSendMedia(deps.Messages, deps.Media, cfg.MaxMediaBytes)))

...
- [ ] 2.1 Tratar `ProtocolMessage` (REVOKE/EDIT) no dispatch com `OnMessageEdit/OnMessageDelete` no sink, verificando testes com eventos sintetizados e fakes atualizados
...
- [ ] 3.1 Implementar cliente HTTP mínimo do Chatwoot (contacts, inboxes, conversations, messages, merge, filter/search, `update_last_seen`) sobre `httptest` fake, verificando paths, headers e corpos contra o fake

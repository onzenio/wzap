...
- [x] 3.4 Adicionar deps pinadas (`phonenumbers`, `x/image`), verificando `go build ./...` limpo
...
- [x] 4.2 Implementar mapeamento de conteúdo (texto, 6 mídias via `media.Storage`, contato, localização, listas, reaction, PIX, ads com thumbnail, markdown, grupos, replies via correlação), verificando cada tipo contra Chatwoot fake
- [x] 4.3 Implementar edit/delete/read-sync e avisos da conversa operacional em pt-BR (conexão, QR com imagem+pairing-code, throttle 30s), verificando cenários contra fakes
- [x] 4.4 Publicar novos subjects de edit/delete via outbox, verificando envelope versionado e consumo pelo worker
...
- [x] 6.2 Implementar import de contatos e mensagens (ordem, dedup `WAID:`, lotes, CTE de FKs, `days_limit`, placeholders), verificando idempotência em reexecução contra Postgres fake/real de teste
- [x] 6.3 Implementar gatilhos (auto pós-pareamento, manual, cron 30min `syncLostMessages`) com avisos no bot, verificando cada gatilho em teste
...
- [x] 7.2 Executar gates (`go vet`, `golangci-lint`, `go test ./...` com Postgres, `go build`, `gofmt -l` vazio), verificando saída limpa
- [x] 7.3 Atualizar README/AGENTS.md e validar a change (`openspec validate`), verificando `openspec status` completo
